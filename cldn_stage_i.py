...
parser = argparse.ArgumentParser(description='CLDN')
...
def main_worker(gpu, args):
    ...
    vgg_k_augmentation = [
        ...
        transforms.RandomApply([CLDN.loader.GaussianBlur([.1, 2.])], p=0.5),
        transforms.RandomHorizontalFlip(),
        transforms.RandomApply([transforms.RandomRotation(10)], p=0.5),
        transforms.TenCrop(80),
        transforms.Lambda(lambda crops: torch.stack([transforms.ToTensor()(crop) for crop in crops])),
    ...
    train_dataset = datasets.ImageFolder(
        traindir,CLDN.loader.TwoCropsTransform(transforms.Compose(vgg_augmentation), transforms.Compose(vgg_k_augmentation))
    )
    ...
    acc_t = evaluate(test_loader, model, args)
